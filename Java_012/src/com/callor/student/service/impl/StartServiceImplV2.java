...
public class StartServiceImplV2 extends StartServiceImplV1 {
	public StartServiceImplV2(StudentService stService) {
		...
		// V1의 생성자를 호출하며 V1 생성자에 있는 코드를 그대로 실행하겟다
		super(stService);
		// ImpV1에서는 St...ServiceImplV1()을 사용하여 stService를 초기화 하였다
		// 하지만 여기에서는 ImplV2를 사용하고 싶다
		// 그럴때는 다시 stService를 ImplV2로 초기화를 하면 된다
	...
	public void mainMenu() {
		...
		for (MenuIndex menu : MenuIndex.values()) {
			...
			item = item.replace("_", " "); // 문자열 안의 글자를 다른거로 바꿔줌
