...
public class ScoreServiceA {
	...
	// 이 변수는 ScoreService 클래스의 소유
	// class 영역에는 여러가지 method 들이 있다
	private ScoreDto[] scores = null;
	private Scanner scan = null;

	/*
	 * 생성자 method(Constructor) 클래스를 사용하여 객체를 선언하고 객체를 (사용하기 위하여) 초기화 할 때 호출(실행)하는
	 * method
	 * 
	 * 클래스 이름과 같고(첫글자 대문자), return type 이 없다. static 도 붙일 수 없다
	 * 
	...
	// 생성자에서 이미 data.txt 파일을 open 하고 읽어들일 준비가 되어 있다
	// 생성자에서 만들어둔 scan 객체를 사용하여 데이터를 읽고 처리를 수행하면 된다
	public void loadScores() {
		int index = 0;
		while (scan.hasNext()) {
			String line = scan.nextLine();
//			System.out.println(line);
			// 읽어들인 한 라인의 데이터를 각 항목별로 분해하여 lines 배열에 담기
	...
	/*
	 * loadScores() method 가 data.txt 파일에서 학생의 성적 data 를 읽어서, scores 배열에 저장해 둔 상태
	 * scores 배열
	 */
	public void printScores() {
		 ...
		 * scores 라는 배열을 선언할 때 어떤 명령(도구)을 사용하여 선언했는가?
